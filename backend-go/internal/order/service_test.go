...
func TestCreateOrderFromCart(t *testing.T) {
    ...
    if err := db.Create(&a).Error; err != nil { t.Fatalf("seed article: %v", err) }
    ...
    if err := db.Create(&mv).Error; err != nil { t.Fatalf("seed variant: %v", err) }

    // Seed cart + item
    c := cart.Cart{UserID: 100, Status: string(cart.CartStatusActive)}
    if err := db.Create(&c).Error; err != nil { t.Fatalf("seed cart: %v", err) }
    ci := cart.CartItem{CartID: c.ID, ArticleID: 1, VariantID: 1, Quantity: 2, PriceAtTime: 1500, OriginalPrice: 1500, CustomData: "{}"}
    if err := db.Create(&ci).Error; err != nil { t.Fatalf("seed cart item: %v", err) }
    ...
    if ord.Status != StatusPending { t.Fatalf("status = %s", ord.Status) }
    if ord.Subtotal != 3000 { t.Fatalf("subtotal = %d", ord.Subtotal) }
    if ord.TaxAmount != 240 { t.Fatalf("tax = %d", ord.TaxAmount) }
    if ord.ShippingAmount != 499 { t.Fatalf("shipping = %d", ord.ShippingAmount) }
    if ord.TotalAmount != 3739 { t.Fatalf("total = %d", ord.TotalAmount) }
    if len(ord.Items) != 2 && len(ord.Items) != 0 {
        ...
        if count != 1 { t.Fatalf("items count = %d", count) }
    }
}
