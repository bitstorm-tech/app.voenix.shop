...
func RegisterRoutes(r *gin.Engine, db *gorm.DB) {
    ...
    type categoryCreate struct { Name string `json:"name"` }
    type categoryUpdate struct { Name *string `json:"name"` }
    ...
    adminSubcats.PUT("/:id", func(c *gin.Context) {
        ...
        if payload.Name != nil { existing.Name = *payload.Name }
        if payload.Description != nil { existing.Description = payload.Description }
        if err := db.Save(&existing).Error; err != nil {
    ...
    adminPrompts.PUT("/:id", func(c *gin.Context) {
        ...
        // Apply fields
        if payload.Title != nil { existing.Title = *payload.Title }
        if payload.PromptText != nil { existing.PromptText = payload.PromptText }
        if payload.CategoryID != nil { existing.CategoryID = payload.CategoryID }
        if payload.SubcategoryID != nil { existing.SubcategoryID = payload.SubcategoryID }
        if payload.Active != nil { existing.Active = *payload.Active }
        if payload.ExampleImageFilename != nil {
            old := existing.ExampleImageFilename
            if old != nil && (payload.ExampleImageFilename == nil || *old != *payload.ExampleImageFilename) {
...
func parseIDs(repeated []string, commaSep string) []int {
    vals := []string{}
    for _, v := range repeated { if v != "" { vals = append(vals, v) } }
    if commaSep != "" {
        parts := strings.Split(commaSep, ",")
        for _, p := range parts { if strings.TrimSpace(p) != "" { vals = append(vals, strings.TrimSpace(p)) } }
    }
    uniq := map[int]struct{}{}
    out := []int{}
    for _, v := range vals {
        if n, err := strconv.Atoi(v); err == nil {
            if _, ok := uniq[n]; !ok { uniq[n] = struct{}{}; out = append(out, n) }
        }
    }
    return out
}
...
func uniqueSlotIDs(slots []promptSlotRef) []int {
    ...
    for _, s := range slots {
        ...
        if _, ok := seen[s.SlotID]; !ok { seen[s.SlotID] = struct{}{}; out = append(out, s.SlotID) }
    }
    return out
}
...
func publicPromptExampleURL(filename *string) string {
    if filename == nil || *filename == "" { return "" }
...
func safeDeletePublicImage(filename, kind string) {
    ...
    if err != nil || strings.TrimSpace(filename) == "" { return }
    var dir string
    switch kind {
    case "slot-variant":
