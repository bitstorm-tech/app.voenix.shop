...
class CartQueryServiceImpl(
     ...
     */
    fun findActiveCartEntityByUserId(userId: Long): Cart? =
        cartRepository.findActiveCartByUserId(userId).orElse(null)
    ...
    fun getActiveCartEntityByUserId(userId: Long): Cart =
        cartRepository.findActiveCartByUserId(userId)
            .orElseThrow { com.jotoai.voenix.shop.common.exception.BadRequestException("No active cart found for user: $userId") }

    /**
     * Internal method to save a cart entity
     * Used by other modules that need to persist cart changes
     */
    fun saveCartEntity(cart: Cart): Cart =
