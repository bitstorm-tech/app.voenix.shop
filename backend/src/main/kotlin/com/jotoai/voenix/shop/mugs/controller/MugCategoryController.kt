...
class MugCategoryController(
    ...
    @GetMapping
    fun getAllCategories(): ResponseEntity<List<MugCategoryDto>> = 
        ResponseEntity.ok(mugCategoryService.getAllCategories())
    
    @GetMapping("/{id}")
    fun getCategoryById(@PathVariable id: Long): ResponseEntity<MugCategoryDto> = 
        ResponseEntity.ok(mugCategoryService.getCategoryById(id))
    
    @GetMapping("/search")
    fun searchCategories(@RequestParam name: String): ResponseEntity<List<MugCategoryDto>> = 
        ResponseEntity.ok(mugCategoryService.searchCategoriesByName(name))
    
    @PostMapping
    fun createCategory(@Valid @RequestBody request: CreateMugCategoryRequest): ResponseEntity<MugCategoryDto> {
    ...
    fun updateCategory(
        ...
        ResponseEntity.ok(mugCategoryService.updateCategory(id, request))
