...
import com.jotoai.voenix.shop.mugs.entity.MugSubCategory
import com.jotoai.voenix.shop.mugs.repository.MugCategoryRepository
import com.jotoai.voenix.shop.mugs.repository.MugSubCategoryRepository
...
class MugSubCategoryService(
    ...
    fun getAllSubCategories(): List<MugSubCategoryDto> = 
        mugSubCategoryRepository.findAll().map { it.toDto() }
    
    fun getSubCategoryById(id: Long): MugSubCategoryDto {
        return mugSubCategoryRepository.findById(id)
            .map { it.toDto() }
    ...
    fun searchSubCategoriesByCategoryAndName(categoryId: Long, name: String): List<MugSubCategoryDto> = 
    ...
    fun createSubCategory(request: CreateMugSubCategoryRequest): MugSubCategoryDto {
        val mugCategory = mugCategoryRepository.findById(request.mugCategoryId)
        ...
        val subCategory = MugSubCategory(
    ...
    @Transactional
    fun updateSubCategory(id: Long, request: UpdateMugSubCategoryRequest): MugSubCategoryDto {
        val subCategory = mugSubCategoryRepository.findById(id)
            .orElseThrow { ResourceNotFoundException("MugSubCategory", "id", id) }
        
        request.mugCategoryId?.let { newCategoryId ->
            val newCategory = mugCategoryRepository.findById(newCategoryId)
