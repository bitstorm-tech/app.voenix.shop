...
class MugController(
    ...
    @GetMapping("/{id}")
    fun getMugById(@PathVariable id: Long): ResponseEntity<MugDto> = 
        ResponseEntity.ok(mugService.getMugById(id))
    
    @GetMapping("/search")
    fun searchMugs(@RequestParam name: String): ResponseEntity<List<MugDto>> = 
        ResponseEntity.ok(mugService.searchMugsByName(name))
    
    @GetMapping("/price-range")
    fun getMugsByPriceRange(
        @RequestParam minPrice: Int,
        @RequestParam maxPrice: Int
    ): ResponseEntity<List<MugDto>> = 
        ResponseEntity.ok(mugService.findMugsByPriceRange(minPrice, maxPrice))
    
    @PostMapping
    fun createMug(@Valid @RequestBody request: CreateMugRequest): ResponseEntity<MugDto> {
    ...
    @DeleteMapping("/{id}")
    fun deleteMug(@PathVariable id: Long): ResponseEntity<Void> {
