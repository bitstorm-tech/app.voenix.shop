...
class MugService(
    ...
    fun searchMugsByName(name: String): List<MugDto> = mugRepository.findByNameContainingIgnoreCaseAndActiveTrue(name).map { it.toDto() }

    fun findMugsByPriceRange(
        minPrice: Int,
        maxPrice: Int,
    ): List<MugDto> = mugRepository.findByPriceBetweenAndActiveTrue(minPrice, maxPrice).map { it.toDto() }
