...
class ShirtVariantApiAdapter(
    ...
    ShirtVariantFacade {
    override fun findByArticleId(articleId: Long): List<ShirtArticleVariantDto> = delegate.findByArticleId(articleId)

    @Transactional
    override fun create(
        articleId: Long,
        request: CreateShirtArticleVariantRequest,
    ): ShirtArticleVariantDto = delegate.create(articleId, request)

    @Transactional
    override fun update(
        variantId: Long,
        request: CreateShirtArticleVariantRequest,
    ): ShirtArticleVariantDto = delegate.update(variantId, request)
    ...
    override fun updateExampleImage(
        variantId: Long,
        filename: String,
    ): ShirtArticleVariantDto = delegate.updateExampleImage(variantId, filename)
}
