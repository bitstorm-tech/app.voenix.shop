...
class SlotTypeService(
    ...
    fun getAllSlotTypes(): List<SlotTypeDto> = 
        slotTypeRepository.findAll().map { it.toDto() }
    ...
    @Transactional
    fun updateSlotType(id: Long, request: UpdateSlotTypeRequest): SlotTypeDto {
        val slotType = slotTypeRepository.findById(id)
            .orElseThrow { ResourceNotFoundException("SlotType", "id", id) }
